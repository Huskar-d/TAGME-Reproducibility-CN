...
class Indexer(object):
    ...
    def index_file(self, file_name):
        ...
        for line in f:
            ...
            if re.search(r'</doc>', line):
                ...
                top_content = article_text[0:min(300,article_len)]
                # ignores null titles
                if wiki_uri is None:
                    print "\tINFO: Null Wikipedia title!"
                # ignores disambiguation pages
                elif (wiki_uri.endswith("(disambiguation)>")) or \
                        ((len(article_text) < 500) and (("may refer to:" in top_content) or ("可以指" in top_content))):
            ...
            for t in tag_iter:
                ...
                if tag == "doc":
                    ...
                    wiki_uri = WikipediaUtils.wiki_title_to_uri(doc_title.group(1).encode('utf8')) if doc_title else None
                if tag == "a":
                    article_text += t.group(1) + t.group(4)  # resolves annotations and replace them with mention
                    #print 'tag a 1:%s,2:%s,3:%s,4:%s'%(t.group(1), t.group(2), t.group(3), t.group(4))
                    # extracts only annotations
                    if self.annot_only:
                        link_title = self.linkRE.search(t.group(2))
                        
                        link_uri = WikipediaUtils.wiki_title_to_uri(unquote(link_title.group(1).encode('utf8'))) if link_title else None
    ...
    def index_files(self, input_dir, output_dir):
        ...
        print "output222:::::%s"%output_dir
        print "input222:::::%s"%input_dir
        for path, dirs, _ in os.walk(input_dir):
            print "process parent dir number:%s"%len(dirs)
            for dir in sorted(dirs):
                print "process parentdir :%s"%dir

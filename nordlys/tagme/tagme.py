...
class Tagme(object):
    ...
    def parse(self):
        ...
        """
        ens = {}
        print 'ngram length :%s'%len(self.query.get_ngrams())
        for ngram in self.query.get_ngrams():
            ...
            if (len(ngram) == 1) or (ngram.isdigit()) or (mention.wiki_occurrences < 2) or (len(ngram.split()) > 6):
                print 'invalid ngram:%s, length:%s, isdigit:%s, wiki_occurrence:%s, word cnt:%s'%(ngram,len(ngram),ngram.isdigit(),mention.wiki_occurrences,len(ngram.split()))
                continue
        ...
        if self.DEBUG: 
            ...
            for m in candidate_entities:
                print 'mention:%s'%m
                for e in candidate_entities[m]:
                    print '  candidate entity:%s'%self.__url_str(e)
        return candidate_entities

    def __url_str(self, e):
            return unquote(str(e)).decode('utf8')
    ...
    def disambiguate(self, candidate_entities):
        ...
        for m_i in candidate_entities.keys():
            ...
            for e_m_i in candidate_entities[m_i].keys():
                if self.DEBUG:
                    print "--candidate entity ", self.__url_str(e_m_i)
                ...
                for m_j in candidate_entities.keys():  # all other mentions
                    ...
                    if self.DEBUG:
                        print 'mention vote of %s:%s'%(self.__url_str(m_j), vote_e_m_j)
        ...
        for m_i in self.rel_scores:
            ...
            for en in top_k_ens:
                ...
                if cmn >= best_cmn:
                    best_en = en
                    best_cmn = cmn
                print '  entity %s:cmn->%s, rel->%s'%(self.__url_str(en),cmn,self.rel_scores[m_i][en])
            disamb_ens[m_i] = best_en
            disamb_ens[m_i] = top_k_ens[0]
    ...
    def __get_link_prob(self, mention):
        ...
        meaning: 
        the probability of mention being an anchor(entity)
        formular:
        num of mention occurred as an anchor / num of mention occurred as plain text
        """
        ...
        print 'mention_freq:%s'%mention_freq
        if mention_freq == 0:
            return 0
    ...
    def __get_vote(self, entity, men_cand_ens):
        ...
        for e_i, cmn in men_cand_ens.iteritems():
            ...
            if self.DEBUG: 
                print "\t", self.__url_str(e_i), " cmn:", cmn, "mw_rel:", mw_rel
            vote += cmn * mw_rel
    ...
    def __get_mw_rel(self, e1, e2):
        ...
        #print 'in link of %s and %s:%s'%(self.__url_str(e1),self.__url_str(e2),ens_in_links)
        if min(ens_in_links) == 0:
            return 0
        conj = self.__get_in_links(en_uris)
        #print 'conjunction of %s and %s:%s'%(self.__url_str(e1),self.__url_str(e2),conj)
        if conj == 0:
            return 0
